...
STRIDED CONV IN PLACE OF POOLING.
...
def FireNet_module_LL(n, bottom, firenet_dict, layer_idx, stride3x3):
    ...
    n.tops[prefix+'conv3x3_2'] = L.Convolution(n.tops[prefix+'conv1x1_1'], kernel_size=3, pad=1, num_output=firenet_dict['conv3x3_2_num_output'], weight_filler=dict(type='xavier'), stride=stride3x3)
...
def FireNet_generic(FireNet_module_func, choose_num_output_func, batch_size, pool_after, s):
    ...
    for layer_idx in xrange(2,10):
        ...
        if layer_idx in p:
            ...
            stride3x3=1
        curr_bottom = FireNet_module_func(n, curr_bottom, firenet_dict, layer_idx, stride3x3) 
...
def get_base_incr_schemes():
    ...
    for i3x3 in [64, 96]:
        base_incr.append({'base_3x3_2':i3x3, 'incr_3x3_2':i3x3, 'in_out_ratio':0.5, 'incr_freq':2})
        #base_incr.append({'base_3x3_2':i3x3, 'incr_3x3_2':i3x3, 'in_out_ratio':1.0, 'incr_freq':2})
        #base_incr.append({'base_3x3_2':i3x3, 'incr_3x3_2':i3x3, 'in_out_ratio':2.0, 'incr_freq':2})
    return base_incr
...
if __name__ == "__main__":
    ...
    p = {1, 2, 4, 8} #conv1, fire2, fire4, fire8
    ...
    for s in base_incr_schemes:
        ...
        out_dir = 'nets/FireNet_LL_8_fireLayers_base_%d_incr_%d_inOutRatio_%0.3f_convStride_1_2_4_8_freq_%d' %(s['base_3x3_2'], 
